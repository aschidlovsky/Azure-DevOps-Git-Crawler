...
return f"[https://dev.azure.com/{env['ORG']}/{env['PROJECT']}/_apis/git/repositories/{env['REPO']}](https://dev.azure.com/{env['ORG']}/{env['PROJECT']}/_apis/git/repositories/{env['REPO']})"
...
def _get_int(key: str, default: int) -> int:
    try:
        v = first_key(body if isinstance(body, dict) else {}, [key])  # exact only
...
def _get_float(key: str, default: float) -> float:
    try:
        v = first_key(body if isinstance(body, dict) else {}, [key])
